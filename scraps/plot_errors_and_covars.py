...
fc_errors_path = data_dir + "fc_covars_%s.npy" % sequence
fc_covars_path = data_dir + "fc_errors_%s.npy" % sequence
...
for i in range(0, 6):
    plt.figure(i)
    plt.plot(fc_errors_abs[:, i], "r")
    plt.plot(-fc_covars_3sig[:, i], "r")
    plt.plot(fc_errors_abs[:, i], "b")
    plt.plot(-fc_covars_3sig[:, i], "b")
