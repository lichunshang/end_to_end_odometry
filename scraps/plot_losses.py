...
losses_to_overlay = [
    (data_dir + "run_results-tag-se3_losses.csv", {"linewidth": 1.0, "color": "r"}, "Training Loss"),
    (data_dir + "run_results-tag-training_loss_val.csv", {"linewidth": 1.0, "color": "b"}, "Validation Loss")
]
...
x = data[:, 1] / 797
y = data[:, 2]
y = tools.smooth(y, window_len=1500, window="hanning")
...
x = data[:, 1] / 24
y = data[:, 2]
y = tools.smooth(y, window_len=50, window="hanning")
plt.plot(x, y[0: len(x)], **loss_to_draw[1], label=loss_to_draw[2])
