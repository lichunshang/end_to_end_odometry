...
def seq_model_inputs(cfg):
    ...
    return inputs, lstm_initial_state, initial_poses, imu_data, ekf_initial_state, ekf_initial_covariance, is_training


def build_seq_model(cfg, inputs, lstm_initial_state, initial_poses, imu_data, ekf_initial_state, ekf_initial_covariance,
                    is_training, get_activations=False,
                    use_initializer=False):
    print("Building CNN...")
    cnn_outputs = cnn_layer(inputs, cnn_model_lidar, is_training, get_activations)

    if use_initializer:
        feed_init_states, feed_ekf_init_state, feed_ekf_init_covar = initializer_layer(cnn_outputs, cfg)
    else:
        feed_init_states = lstm_initial_state
        feed_ekf_init_state = ekf_initial_state
        feed_ekf_init_covar = ekf_initial_covariance
